...
{"fmt":"airqr1","tid":305419896,"name":"report.pdf","size":1048576,
 "blen":2900,"k":362,"zstd":0,"grid":4}
```

- `tid`：§2 transfer_id 的十进制形式；`name`：原始文件名（无路径）；`size`：原始字节数；
  `blen`：§2 block_len；`k`：源块数；`zstd`：0/1；`grid`：一屏 cell 数（§2 GridGeom：
  1/2/4/6/8），接收端按此切分解码单元。`grid` 缺失时按 0（unknown）处理——
  grid 之前的发送端保持合法，新字段只增不改，旧接收端忽略未知 key。
